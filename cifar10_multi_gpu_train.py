...
# ==============================================================================
"""Train CIFAR-10 using multiple GPUs with synchronous updates.

  Accuracy:
    ~ 86% after 100k steps (256 epochs of data)
"""
...
tf.app.flage.DEFINE_boolean('log_device_placement', False,
...
def average_gradients(tower_grads):
  ...
  for grad_and_vars in zip(*tower_grads):
    ...
    # ((grad0_gpu0, var0_gpu0), ... (gradN_gpuN, varN_gpuN))
    grads = []
    for g, _ in grad_and_vars:
      # Add 0 dimension to the gradients to represent the tower
      expanded_g = tf.expand_dims(g, 0)

      # Append on a 'tower' dimension which will be averaged over below
  ...
  def train():
    ...
    with tf.Graph().as_default, tf.device('/cpu:0'):
      ...
      for i in xrange(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (cifar10.TOWER_NAME, i)) as scope:
            ...
            # Calculate the gradients for the bach of data on this tower
            grads = opt.compute_gradients(loss)

            # Keep track of the gradients across all towers
            tower_grads.append(grads)

      # Calculate the mean of each gradient. Note that this is a
      ...
      # Build a summary operation from the last tower summaries
      ...
      # to True to build towers on GPU, as some of the ops do not have
      # GPU implementations
      ...
      for step in xrange(FLAGS.max_steps):
        ...
        if step % 10 == 0:
          ...
          format_str('%s: step %d, loss = %.2f (%.1f exmaples/sec; %.3f '
                     'sec/batch)')
