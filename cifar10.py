...
# If a model is trained with multiple GPUs, prefix all Op names with tower_nam
...
def distorted_inputs():
  ...
  if FLAGS.use_fp16:
    ...
    labels = tf.cast(lables, tf.float16)
...
def inputs(eval_data):
  ...
  Args:
    eval_data: boolean, indicating wheter to use train or eval data set
...
def loss(logits, labels):
  ...
  Args:
    ...
    lables: Labels from distorted_inputs or inputs().
            1-D Tensor of shape [batch_size]

  Returns:
    Loss Tensor of type float
  """
  # Calulate the average cross entropy loss across the batch
...
def _add_loss_summaries(total_loss):
  ...
  Generates moving average for all losses and associates summaries for
...
def maybe_download_and_extract():
  """Download and extract CIFAR-10 tarball"""
